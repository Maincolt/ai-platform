...
> Last updated: 2026-08-16 | ADR-0026 (autonomous team agents, policy-only) + `scrum.status` (ADR-0027, ADR-0026 Phase 1): a tenth capability reading a live GitHub Projects v2 board via one deterministic fetch + one AI call, deployed but not yet live-verified end to end (no real board/PAT configured yet) | Status: In progress
...
[README.md](README.md)). Twenty-seven Accepted ADRs govern the platform.
...
| — | `scrum.status` — a Read-Only, Live Scrum-Board Status Capability | 🔄 In progress (deployed, pending real board/PAT) | [ADR-0027](docs/architecture/decisions/ADR-0027-scrum-status-capability.md) (Accepted): ADR-0026's Phase 1 — the platform's tenth capability and ninth AI-backed one, structurally identical to `ui.review`'s fetch-then-single-AI-call shape, but fetching an authenticated GitHub Projects v2 board via GraphQL instead of an unauthenticated Playwright page load. New `ProjectBoardPort`/`GitHubProjectsBoardReader`, a new `read:project`-scoped PAT credential class, and (unlike `ui.review`) config-driven rather than hardcoded project coordinates, since there's no SSRF-shaped risk to close off for an authenticated API call. `{location, summary, severity}` findings. One PR (#56, domain/contract layer + deployment wiring together), live-verified on the Mac Docker host: all ten capabilities reached `READY` on the first check (confirmed visually via a Playwright screenshot showing "10 / 10 online" with zero frontend changes needed), the full 183-case live Kafka ACL matrix passes, and a real submission against the placeholder credentials correctly failed closed with `PROJECT_BOARD_FETCH_FAILED` carrying GitHub's own real `401 Bad credentials` response — proving the pipeline is wired correctly end to end short of the credential itself. **Genuine board-derived verification still pending**: the repository has no populated GitHub Projects v2 board yet, so `scrum-status-agent` runs against placeholder credentials until the repository owner creates a real board and supplies a real PAT. |
...
- **`scrum.status` v1.0** (`src/ai_platform/agents/scrum_status_agent/`, [ADR-0027](docs/architecture/decisions/ADR-0027-scrum-status-capability.md), ADR-0026 Phase 1), the platform's tenth capability and ninth AI-backed one: the same fetch-then-single-AI-call shape `ui.review` established — a deterministic, read-only GitHub Projects v2 GraphQL fetch (`ProjectBoardPort`/`GitHubProjectsBoardReader`, an injectable-transport seam for fast deterministic unit tests with `httpx.MockTransport`, no real network needed) feeds one AI Router call, `{location, summary, severity}` findings out. Unlike `ui.review`, the fetch target (project owner/number) is ordinary config, not a hardcoded constant — an authenticated, PAT-scoped API call has no SSRF-shaped risk to close off. Its own `scrum-status-agent` Compose service/Kafka principals/Registry binding, live-verified on the Mac Docker host (`READY`, correct fail-closed `PROJECT_BOARD_FETCH_FAILED` against placeholder credentials); genuine board-derived verification still pending a real board + PAT (ADR-0027's Implementation Status).
- **[ADR-0026](docs/architecture/decisions/ADR-0026-autonomous-team-agents.md): Autonomous Team Agents** (Accepted, policy/architecture only) — authorizes three future roles to take real, hard-to-reverse actions with no per-action human approval, narrowly amending `SECURITY.md`'s Human Approval for High-Impact Actions policy. `scrum.status` above is its Phase 1; Phases 2+ (write access, a bounded per-role tool-calling loop, a durable audit trail, a platform-wide kill switch, a spend/rate cap) are future work, not yet built.
- **Real Anthropic (primary) and OpenAI (fallback) AI provider credentials are now configured** (`infrastructure/compose/secrets/`, git-ignored) and eight of the nine AI-backed capabilities are live-verified with genuine model completions — real Anthropic (Claude Haiku 4.5) round-trips for `text.summarize`/`code.review`/`ui.review`/`architecture.review`/`data.analysis`/`technical.review`/`assignment.route`/`security.review` all confirmed working end to end, including a real-model `code.review` catching a planted hardcoded password + SQL injection, a real-model `ui.review` catching genuine accessibility issues on the live dashboard, a real-model `data.analysis` correctly correlating a four-metric anomaly across an otherwise-stable dataset, a real-model `technical.review` catching eight genuine schema/API design issues in a deliberately under-specified proposal, a real-model `assignment.route` + `submit-assignment.py` correctly splitting a mixed assignment across `technical.review`/`data.analysis` end to end, and a real-model `security.review` catching two SQL injections, a hardcoded API key, and a missing auth check in a deliberately vulnerable snippet. `scrum.status` uses the same credentials but is not yet live-verified (no real project board configured yet). The OpenAI fallback path itself has not yet been exercised (Anthropic has never failed in testing).
