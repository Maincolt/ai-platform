...
> Last updated: 2026-08-19 | `crypto-market-agent`/`forex-market-agent` (ADR-0035/ADR-0036): the platform's first fully action-free autonomous roles — no credential, no external write path of any kind, findings-only — implemented and tests-passing, not yet deployed | Status: In progress
...
| — | `backend-specialist-agent` — A Third Domain Review Role | ✅ Done | [ADR-0034](docs/architecture/decisions/ADR-0034-backend-specialist-agent.md) (Accepted): a sixth autonomous role, closing the gap ADR-0033 itself flagged as the clearest remaining one — a Python backend reviewer, filtered to `src/ai_platform/`. Zero new code beyond configuration/wiring: reuses `DomainReviewAgent`/`_pull_request_review_shared.py` (ADR-0033) unchanged, a third `build_*_process()` composition function passing different role/domain-label/path-prefix literals. Deliberately overlaps `postgres-specialist-agent`'s persistence paths (the filter has no exclusion mechanism, and two independent review comments on the same PR is treated as layered depth, not a bug). Same re-amend-`SECURITY.md`-by-name requirement as every new role. Repository owner supplied a real `repo`-scoped PAT before first deployment — skipped the placeholder step every prior role used, deployed directly with the real credential; a live cycle showed a genuine `200 OK` fetch, correctly reporting nothing open to review. |
| — | `crypto-market-agent` + `forex-market-agent` — Two Independent, Action-Free Market-Observer Roles | 🔄 In progress | [ADR-0035](docs/architecture/decisions/ADR-0035-crypto-market-agent.md)/[ADR-0036](docs/architecture/decisions/ADR-0036-forex-market-agent.md) (both Accepted): the seventh and eighth autonomous roles, and the first with no external write path of any kind — `PeriodicService`-driven, hourly, fetch a configured watchlist from a free unauthenticated public API (Binance's public 24h-ticker endpoint for crypto, Frankfurter's ECB reference rates for forex), one AI Router call, strict-parse a findings list, record each as an `agent.autonomous_actions` row (`action_type="record_finding"`). No credential, no new migration, no `SECURITY.md` amendment (no action exists to exempt). Deliberately independent implementations at the repository owner's explicit instruction — no shared package between the two despite the similar shape, only the pure helpers every autonomous role already shares (`_autonomous_shared.py`). `src/ai_platform/agents/crypto_market_agent/`, `src/ai_platform/agents/forex_market_agent/`, `CryptoMarketRuntimeConfig`/`ForexMarketRuntimeConfig`, `build_crypto_market_process`/`build_forex_market_process`, two new Compose services, two new dashboard `KNOWN_ROLES` entries. Ruff, BasedPyright (strict), and the full unit suite (707 tests, including 33 new) all pass. Not yet deployed to the Mac Docker host or live-verified. |
