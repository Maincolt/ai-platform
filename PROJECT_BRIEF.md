...
[README.md](README.md)). Twenty-eight Accepted ADRs govern the platform.
...
| — | `scrum-master-agent` — ADR-0026 Phase 2, Real Autonomous Board Write Access | 🔄 In progress | [ADR-0028](docs/architecture/decisions/ADR-0028-scrum-master-agent-phase-2.md) (Accepted): the platform's first Agent not driven by a Workflow submission — no `ExecuteTask` consumption, no Kafka wiring at all, no Capability Registry binding. A `PeriodicService` (reused unchanged from the Orchestrator's `DeadlineReconciler`) wakes it hourly to propose and dispatch real write actions (`set_status`/`add_comment`/`create_draft_item` — a deliberately narrower MVP than ADR-0026's full six-action target) against the same board `scrum.status` reads, bounded by a new platform-wide kill switch, a 10-actions/$1-per-day hard cap, and a durable append-only audit log (`agent.autonomous_actions`, migration 0009, agent schema version 4→5) — all new, all DB-backed, all checked every cycle. A separate, `project`+`repo`-scoped PAT from `scrum-status-agent`'s (per-role least privilege). Domain/contract layer and deployment wiring landed; live deployment and the first real autonomous action against the repository owner's real board are next. |
