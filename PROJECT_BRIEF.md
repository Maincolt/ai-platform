...
> Last updated: 2026-08-15 | `architecture.review` (ADR-0020) and `data.analysis` (ADR-0021) added, live-verified on the Mac Docker host; the platform now has six capabilities | Status: In progress
...
[README.md](README.md)). Twenty-one Accepted ADRs govern the platform.
...
it depends on. Four more advisory, AI-backed capabilities followed the
...
configured** and all five AI-backed capabilities have been live-verified
...
- **`data.analysis` v1.0** (`src/ai_platform/agents/data_analysis_agent/`, [ADR-0021](docs/architecture/decisions/ADR-0021-data-analysis-capability.md)), the platform's sixth capability and fifth AI-backed one: structurally identical to `architecture.review` — a dataset excerpt/metrics summary/usage report in, `{metric, summary, severity}` findings out. Closes out five of the six ADR-0018 Decision 2 personas (only Technical Architect remains unbuilt). Its own `data-analysis-agent` Compose service/Kafka principals/Registry binding.
- **Real Anthropic (primary) and OpenAI (fallback) AI provider credentials are now configured** (`infrastructure/compose/secrets/`, git-ignored) and all five AI-backed capabilities are live-verified with genuine model completions — real Anthropic (Claude Haiku 4.5) round-trips for `text.summarize`/`code.review`/`ui.review`/`architecture.review`/`data.analysis` all confirmed working end to end, including a real-model `code.review` catching a planted hardcoded password + SQL injection, a real-model `ui.review` catching genuine accessibility issues on the live dashboard, and a real-model `data.analysis` correctly correlating a four-metric anomaly across an otherwise-stable dataset. The OpenAI fallback path itself has not yet been exercised (Anthropic has never failed in testing).
...
- A seventh platform capability, if wanted: of the six personas ADR-0018 Decision 2 assessed as fitting the bounded-advisory shape, only Technical Architect remains unbuilt after `architecture.review` (ADR-0020) and `data.analysis` (ADR-0021).
