...
> Last updated: 2026-08-15 | `technical.review` (ADR-0022) added, live-verified on the Mac Docker host; the platform now has seven capabilities, closing out all six ADR-0018 Decision 2 personas | Status: In progress
...
[README.md](README.md)). Twenty-two Accepted ADRs govern the platform.
...
found fitting the bounded-advisory model.
**Real Anthropic (primary) and OpenAI (fallback) credentials are now
configured** and all six AI-backed capabilities have been live-verified
...
- **Real Anthropic (primary) and OpenAI (fallback) AI provider credentials are now configured** (`infrastructure/compose/secrets/`, git-ignored) and all six AI-backed capabilities are live-verified with genuine model completions — real Anthropic (Claude Haiku 4.5) round-trips for `text.summarize`/`code.review`/`ui.review`/`architecture.review`/`data.analysis`/`technical.review` all confirmed working end to end, including a real-model `code.review` catching a planted hardcoded password + SQL injection, a real-model `ui.review` catching genuine accessibility issues on the live dashboard, a real-model `data.analysis` correctly correlating a four-metric anomaly across an otherwise-stable dataset, and a real-model `technical.review` catching eight genuine schema/API design issues in a deliberately under-specified proposal. The OpenAI fallback path itself has not yet been exercised (Anthropic has never failed in testing).
...
- An eighth platform capability, if wanted: all six personas ADR-0018 Decision 2 assessed as fitting the bounded-advisory shape are now built. Anything further needs a fresh fit assessment (per ADR-0018 Decision 2's own "does not fit" list) rather than reuse of an existing pre-approved slot.
