"""Component tests for the asynchronous Architecture Review Agent execution
lifecycle (ADR-0020), mirroring
`tests/component/agents/review_agent/test_review_agent_lifecycle.py`."""
...
from ai_platform.agents.architecture_review_agent.agent import (
    ...
    ArchitectureReviewAgentDisposition,
)
...
_ONE_FINDING_JSON = json.dumps(
    [{"section": "Decision 2", "summary": "Missing rollback plan.", "severity": "medium"}]
)
...
def _context(
    *,
    ...
    input_text: str = "# ADR-0021: Proposed Change\n\n## Decision\n\nDo the thing.\n",
    capability_name: str = "architecture.review",
...
) -> tuple[ArchitectureReviewAgent, InMemoryAgentPersistence]:
    persistence = persistence or InMemoryAgentPersistence()
    agent = ArchitectureReviewAgent(
        environment="local-development",
        agent_deployment_id=AgentId("architecture-review-agent"),
        agent_component="architecture-review-agent",
...
def test_successful_completion_commits_completed_outcome_with_usage() -> None:
    ...
    assert result.disposition == ArchitectureReviewAgentDisposition.COMPLETED
...
def test_malformed_provider_output_commits_failed_outcome_but_still_attaches_usage() -> None:
    ...
    assert result.disposition == ArchitectureReviewAgentDisposition.FAILED
    assert result.outcome.result_data is None
    assert result.outcome.failure_code == "MALFORMED_ARCHITECTURE_REVIEW_OUTPUT"
...
def test_deadline_already_expired_never_claims_or_calls_router() -> None:
    ...
    assert (
        result.disposition == ArchitectureReviewAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
    )
...
def test_redelivery_with_resolved_outcome_short_circuits_without_new_claim() -> None:
    ...
    assert first.disposition == ArchitectureReviewAgentDisposition.COMPLETED
    assert second.disposition == ArchitectureReviewAgentDisposition.DUPLICATE_RESOLVED
