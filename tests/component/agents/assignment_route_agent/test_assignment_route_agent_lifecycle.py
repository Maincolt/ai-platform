"""Component tests for the asynchronous Assignment Route Agent execution
lifecycle (ADR-0023), mirroring
`tests/component/agents/technical_review_agent/test_technical_review_agent_lifecycle.py`."""
...
from ai_platform.agents.assignment_route_agent.agent import (
    AssignmentRouteAgent,
    AssignmentRouteAgentDisposition,
)
...
_ONE_RECOMMENDATION_JSON = json.dumps(
    [
        {
...
def _context(
    *,
    ...
    input_text: str = "We want to add a new caching layer in front of the AI Router.",
    capability_name: str = "assignment.route",
...
) -> tuple[AssignmentRouteAgent, InMemoryAgentPersistence]:
    persistence = persistence or InMemoryAgentPersistence()
    agent = AssignmentRouteAgent(
        environment="local-development",
        agent_deployment_id=AgentId("assignment-route-agent"),
        agent_component="assignment-route-agent",
...
def test_successful_completion_commits_completed_outcome_with_usage() -> None:
    router = FakeAIRouter(
        result=AICompletionResult(
            output_text=_ONE_RECOMMENDATION_JSON,
    ...
    assert result.disposition == AssignmentRouteAgentDisposition.COMPLETED
    ...
    assert payload["payload"]["result"]["assignments"][0]["capability"] == "architecture.review"


def test_empty_recommendation_list_is_a_valid_completion() -> None:
    router = FakeAIRouter(
        result=AICompletionResult(
            output_text="[]",
            usage=AICompletionUsage(
    ...
    assert result.disposition == AssignmentRouteAgentDisposition.COMPLETED
    assert result.outcome.result_data == {"assignments": []}
...
def test_malformed_provider_output_commits_failed_outcome_but_still_attaches_usage() -> None:
    ...
    router = FakeAIRouter(
        result=AICompletionResult(
            output_text="Sure, here is my routing decision: send it to everyone!",
            usage=AICompletionUsage(
                provider="anthropic",
                model="m",
                input_tokens=12,
    ...
    assert result.disposition == AssignmentRouteAgentDisposition.FAILED
    assert result.outcome.result_data is None
    assert result.outcome.failure_code == "MALFORMED_ASSIGNMENT_ROUTE_OUTPUT"
    assert result.outcome.summary is not None
    assert "send it to everyone" not in result.outcome.summary
    usage = persistence.provider_call_usage[TaskAttemptId("attempt-1")]
    assert usage.provider == "anthropic"
...
def test_deadline_already_expired_never_claims_or_calls_router() -> None:
    ...
    assert result.disposition == AssignmentRouteAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
...
def test_redelivery_with_resolved_outcome_short_circuits_without_new_claim() -> None:
    ...
    assert first.disposition == AssignmentRouteAgentDisposition.COMPLETED
    assert second.disposition == AssignmentRouteAgentDisposition.DUPLICATE_RESOLVED
