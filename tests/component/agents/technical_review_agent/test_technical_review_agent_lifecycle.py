"""Component tests for the asynchronous Technical Review Agent execution
lifecycle (ADR-0022), mirroring
`tests/component/agents/data_analysis_agent/test_data_analysis_agent_lifecycle.py`."""
...
from ai_platform.agents.technical_review_agent.agent import (
    ...
    TechnicalReviewAgentDisposition,
)
    ...
    [
        {
            ...
            "summary": "No unique constraint on email.",
            "severity": "medium",
        }
    ]
)
...
def _context(
    *,
    ...
    input_text: str = "Table users(id uuid pk, email text, created_at timestamptz)",
    capability_name: str = "technical.review",
...
) -> tuple[TechnicalReviewAgent, InMemoryAgentPersistence]:
    persistence = persistence or InMemoryAgentPersistence()
    agent = TechnicalReviewAgent(
        environment="local-development",
        agent_deployment_id=AgentId("technical-review-agent"),
        agent_component="technical-review-agent",
...
def test_successful_completion_commits_completed_outcome_with_usage() -> None:
    ...
    assert result.disposition == TechnicalReviewAgentDisposition.COMPLETED
...
def test_malformed_provider_output_commits_failed_outcome_but_still_attaches_usage() -> None:
    ...
    router = FakeAIRouter(
        result=AICompletionResult(
            output_text="Sure, here is my review: looks buildable to me!",
            usage=AICompletionUsage(
                provider="anthropic",
                model="m",
                input_tokens=12,
    ...
    assert result.disposition == TechnicalReviewAgentDisposition.FAILED
    assert result.outcome.result_data is None
    assert result.outcome.failure_code == "MALFORMED_TECHNICAL_REVIEW_OUTPUT"
    assert result.outcome.summary is not None
    assert "looks buildable to me" not in result.outcome.summary
    usage = persistence.provider_call_usage[TaskAttemptId("attempt-1")]
    assert usage.provider == "anthropic"
...
def test_deadline_already_expired_never_claims_or_calls_router() -> None:
    ...
    assert result.disposition == TechnicalReviewAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
...
def test_redelivery_with_resolved_outcome_short_circuits_without_new_claim() -> None:
    ...
    assert first.disposition == TechnicalReviewAgentDisposition.COMPLETED
    assert second.disposition == TechnicalReviewAgentDisposition.DUPLICATE_RESOLVED
