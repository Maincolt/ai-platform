"""Component tests for the asynchronous Security Review Agent execution
lifecycle (ADR-0025), mirroring
...
from ai_platform.agents.security_review_agent.agent import (
    SecurityReviewAgent,
    SecurityReviewAgentDisposition,
)
...
def _context(
    *,
    ...
    input_text: str = "API_KEY = 'sk-live-abc123'  # hardcoded for local dev",
    capability_name: str = "security.review",
...
) -> tuple[SecurityReviewAgent, InMemoryAgentPersistence]:
    persistence = persistence or InMemoryAgentPersistence()
    agent = SecurityReviewAgent(
        environment="local-development",
        agent_deployment_id=AgentId("security-review-agent"),
        agent_component="security-review-agent",
...
def test_successful_completion_commits_completed_outcome_with_usage() -> None:
    ...
    assert result.disposition == SecurityReviewAgentDisposition.COMPLETED
...
def test_malformed_provider_output_commits_failed_outcome_but_still_attaches_usage() -> None:
    ...
    router = FakeAIRouter(
        result=AICompletionResult(
            output_text="Sure, here is my security review: looks fine to me!",
            usage=AICompletionUsage(
                provider="anthropic",
                model="m",
                input_tokens=12,
    ...
    assert result.disposition == SecurityReviewAgentDisposition.FAILED
    assert result.outcome.result_data is None
    assert result.outcome.failure_code == "MALFORMED_SECURITY_REVIEW_OUTPUT"
...
def test_deadline_already_expired_never_claims_or_calls_router() -> None:
    ...
    assert result.disposition == SecurityReviewAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
...
def test_redelivery_with_resolved_outcome_short_circuits_without_new_claim() -> None:
    ...
    assert first.disposition == SecurityReviewAgentDisposition.COMPLETED
    assert second.disposition == SecurityReviewAgentDisposition.DUPLICATE_RESOLVED
