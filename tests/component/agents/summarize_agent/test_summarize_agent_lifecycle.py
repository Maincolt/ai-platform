"""Component tests for the asynchronous Summarize Agent execution lifecycle
(ADR-0014 Section 5)."""
...
from ai_platform.agents.summarize_agent.agent import (
    SummarizeAgent,
    SummarizeAgentDisposition,
)
...
class FakeAIRouter:
    ...
    never used in this repository's default test suite (Sprint 9 scope)."""
...
def _context(
    *,
    ...
    capability_name: str = "text.summarize",
...
) -> tuple[SummarizeAgent, InMemoryAgentPersistence]:
    persistence = persistence or InMemoryAgentPersistence()
    agent = SummarizeAgent(
        environment="local-development",
        agent_deployment_id=AgentId("summarize-agent"),
        agent_component="summarize-agent",
...
def test_successful_completion_commits_completed_outcome_with_usage() -> None:
    ...
    assert result.disposition == SummarizeAgentDisposition.COMPLETED
    assert result.outcome.result_data == {"summary": "a short summary"}
    ...
    assert payload["payload"]["result"] == {"summary": "a short summary"}
...
def test_provider_failure_commits_failed_outcome_with_provider_failure_code() -> None:
    ...
    assert result.disposition == SummarizeAgentDisposition.FAILED
    ...
    assert payload["payload"]["failure_code"] == "ALL_PROVIDERS_EXHAUSTED"
...
def test_deadline_already_expired_never_claims_or_calls_router() -> None:
    ...
    assert result.disposition == SummarizeAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
...
def test_redelivery_with_resolved_outcome_short_circuits_without_new_claim() -> None:
    ...
    assert first.disposition == SummarizeAgentDisposition.COMPLETED
    assert second.disposition == SummarizeAgentDisposition.DUPLICATE_RESOLVED
