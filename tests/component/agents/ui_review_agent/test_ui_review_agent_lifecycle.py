"""Component tests for the asynchronous UI Review Agent execution lifecycle
(ADR-0019), mirroring `tests/component/agents/review_agent/test_review_agent_lifecycle.py`.

Two cases have no `review_agent` analog, since they're specific to this
capability's pre-AI-call capture phase (ADR-0019 Decision 1/4):
`test_disallowed_review_target_...` and `test_capture_failure_...` --
both must reject before any provider-call claim is taken and without
ever calling the AI Router.
"""
...
from ai_platform.agents.ui_review_agent.agent import UiReviewAgent, UiReviewAgentDisposition
from ai_platform.agents.ui_review_agent.capture import ConsoleMessage, PageCapture
...
_ALLOWED_TARGET = "http://platform:80"

_ONE_FINDING_JSON = json.dumps(
    [{"area": "console", "summary": "Uncaught TypeError.", "severity": "medium"}]
)

_DEFAULT_CAPTURE = PageCapture(
    url=_ALLOWED_TARGET,
    http_status=200,
    title="Agent Status Dashboard",
    visible_text="Agents Online: 3/3",
    console_messages=(ConsoleMessage(level="error", text="Uncaught TypeError"),),
...
@dataclass
class FakePageCapture:
    """A fixed-response fake `PageCapturePort`; no real browser is ever
    launched in this repository's default test suite."""

    capture_result: PageCapture | None = None
...
class _UnreachablePageCapture:
    async def capture(self, url: str) -> PageCapture:
        del url
        raise AssertionError("Playwright must not be invoked on this path")
...
def _context(
    *,
    ...
    input_text: str = _ALLOWED_TARGET,
    capability_name: str = "ui.review",
...
def _build_agent(
    *,
    ...
    page_capture: FakePageCapture | _UnreachablePageCapture | None = None,
    allowed_review_target: str = _ALLOWED_TARGET,
    max_output_tokens: int = 256,
) -> tuple[UiReviewAgent, InMemoryAgentPersistence]:
    persistence = persistence or InMemoryAgentPersistence()
    agent = UiReviewAgent(
        environment="local-development",
        agent_deployment_id=AgentId("ui-review-agent"),
        agent_component="ui-review-agent",
        ...
        page_capture=(
            ...
            else FakePageCapture(capture_result=_DEFAULT_CAPTURE)
        ),
        allowed_review_target=allowed_review_target,
...
def test_successful_completion_commits_completed_outcome_with_usage() -> None:
    ...
    assert result.disposition == UiReviewAgentDisposition.COMPLETED
    assert result.outcome.result_data == {
        "findings": [{"area": "console", "summary": "Uncaught TypeError.", "severity": "medium"}]
    }
...
def test_malformed_provider_output_commits_failed_outcome_but_still_attaches_usage() -> None:
    ...
    router = FakeAIRouter(
        result=AICompletionResult(
            output_text="Sure, here is my review: looks fine to me!",
            usage=AICompletionUsage(
                provider="anthropic",
                model="m",
                input_tokens=12,
    ...
    assert result.disposition == UiReviewAgentDisposition.FAILED
    assert result.outcome.result_data is None
    assert result.outcome.failure_code == "MALFORMED_UI_REVIEW_OUTPUT"
    assert result.outcome.summary is not None
    assert "looks fine to me" not in result.outcome.summary
    usage = persistence.provider_call_usage[TaskAttemptId("attempt-1")]
    assert usage.provider == "anthropic"
...
def test_deadline_already_expired_never_claims_captures_or_calls_router() -> None:
    agent, persistence = _build_agent(page_capture=_UnreachablePageCapture())
    ...
    assert result.disposition == UiReviewAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
...
def test_disallowed_review_target_commits_failed_outcome_without_capture_or_claim() -> None:
    """ADR-0019 Decision 4: a target other than the hardcoded allowed
    review target must fail closed before Playwright ever navigates and
    before any provider-call claim is taken -- no AI Router spend, no
    browser launch, for an input the platform will never act on."""
    agent, persistence = _build_agent(page_capture=_UnreachablePageCapture())

    result = _run(agent.handle(_context(input_text="http://evil.example.com"), now=NOW))

    assert result.disposition == UiReviewAgentDisposition.FAILED
    assert result.outcome.failure_code == "DISALLOWED_REVIEW_TARGET"
    assert persistence.provider_call_claims == {}
    payload = json.loads(
        persistence.completed_work[TaskAttemptId("attempt-1")].event_outbox.payload_bytes
    )
...
def test_capture_failure_commits_failed_outcome_without_claim_or_router_call() -> None:
    """A Playwright navigation failure must fail closed before any
    provider-call claim is taken -- no AI Router spend for a page that was
    never successfully captured."""
    router = FakeAIRouter(
        result=AICompletionResult(
            output_text=_ONE_FINDING_JSON,
            usage=AICompletionUsage(
    ...
    capture = FakePageCapture(failure=CaptureFailedError("navigation timeout"))
    agent, persistence = _build_agent(ai_router=router, page_capture=capture)

    result = _run(agent.handle(_context(), now=NOW))

    assert result.disposition == UiReviewAgentDisposition.FAILED
    assert result.outcome.failure_code == "PAGE_CAPTURE_FAILED"
    assert "navigation timeout" in (result.outcome.summary or "")
    assert persistence.provider_call_claims == {}
    assert router.calls == []
...
def test_redelivery_with_resolved_outcome_short_circuits_without_new_claim() -> None:
    ...
    assert first.disposition == UiReviewAgentDisposition.COMPLETED
    assert second.disposition == UiReviewAgentDisposition.DUPLICATE_RESOLVED
