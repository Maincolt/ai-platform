"""Component tests for the asynchronous Data Analysis Agent execution
lifecycle (ADR-0021), mirroring
`tests/component/agents/architecture_review_agent/test_architecture_review_agent_lifecycle.py`."""
...
def _context(
    *,
    ...
    input_text: str = "metric,value\np95_latency_ms,420\nmau,18000\n",
    capability_name: str = "data.analysis",
...
) -> tuple[DataAnalysisAgent, InMemoryAgentPersistence]:
    persistence = persistence or InMemoryAgentPersistence()
    agent = DataAnalysisAgent(
        environment="local-development",
        agent_deployment_id=AgentId("data-analysis-agent"),
        agent_component="data-analysis-agent",
...
def test_successful_completion_commits_completed_outcome_with_usage() -> None:
    ...
    assert result.disposition == DataAnalysisAgentDisposition.COMPLETED
...
def test_malformed_provider_output_commits_failed_outcome_but_still_attaches_usage() -> None:
    ...
    router = FakeAIRouter(
        result=AICompletionResult(
            output_text="Sure, here is my analysis: everything looks fine!",
            usage=AICompletionUsage(
                provider="anthropic",
                model="m",
                input_tokens=12,
    ...
    assert result.disposition == DataAnalysisAgentDisposition.FAILED
    assert result.outcome.result_data is None
    assert result.outcome.failure_code == "MALFORMED_DATA_ANALYSIS_OUTPUT"
    assert result.outcome.summary is not None
    assert "everything looks fine" not in result.outcome.summary
    usage = persistence.provider_call_usage[TaskAttemptId("attempt-1")]
    assert usage.provider == "anthropic"
...
def test_deadline_already_expired_never_claims_or_calls_router() -> None:
    ...
    assert result.disposition == DataAnalysisAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
...
def test_redelivery_with_resolved_outcome_short_circuits_without_new_claim() -> None:
    ...
    assert first.disposition == DataAnalysisAgentDisposition.COMPLETED
    assert second.disposition == DataAnalysisAgentDisposition.DUPLICATE_RESOLVED
