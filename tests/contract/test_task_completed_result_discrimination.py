...
def test_technical_review_result_validates_against_the_findings_branch() -> None:
    message = _message(
...
def test_technical_review_capability_missing_findings_is_rejected() -> None:
    message = _message(capability="technical.review", result={"summary": "not a findings list"})
    with pytest.raises(ValidationError):
        _validator().validate(message)  # pyright: ignore[reportUnknownMemberType]


def test_technical_review_finding_with_a_data_analysis_metric_key_is_rejected() -> None:
    """`technical.review`'s findings shape has no `metric` concept -- it
    must not accidentally validate against `data.analysis`'s branch."""
    message = _message(
        capability="technical.review",
        result={"findings": [{"metric": "p95 latency", "summary": "x", "severity": "low"}]},
    )
    with pytest.raises(ValidationError):
        _validator().validate(message)  # pyright: ignore[reportUnknownMemberType]


def test_technical_review_finding_with_an_invalid_severity_is_rejected() -> None:
    message = _message(
        capability="technical.review",
        result={"findings": [{"component": "users table", "summary": "x", "severity": "critical"}]},
    )
    with pytest.raises(ValidationError):
        _validator().validate(message)  # pyright: ignore[reportUnknownMemberType]


def test_technical_review_finding_with_an_extra_field_is_rejected() -> None:
    message = _message(
        capability="technical.review",
        result={
            "findings": [
...
def test_technical_review_result_does_not_accept_the_summarize_capabilitys_field() -> None:
