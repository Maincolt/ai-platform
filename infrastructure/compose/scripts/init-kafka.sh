...
# summarize-agent-consumer) receives only the topic and consumer-group
# access its role requires, matching ADR-0005 Section 17 (Orchestrator writes
# commands and reads outcomes; each Agent class reads only its own
# capability-scoped commands and writes outcomes; quarantine access stays
# with the consumer that detects the rejection).
#
