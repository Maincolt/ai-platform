"""Assignment Route Agent domain errors (ADR-0007 Section 2, ADR-0023).
...
`ai_platform.agents.technical_review_agent.errors` (renamed base
exception) rather than imported, per this module's self-contained-deployable
...
class AssignmentRouteAgentError(Exception):
    """Base class for Assignment Route Agent domain errors."""


class CapabilityMismatchError(AssignmentRouteAgentError):
...
class CommandIdentityConflictError(AssignmentRouteAgentError):
...
class CommandIntegrityError(AssignmentRouteAgentError):
...
class MissingOutcomeInvariantError(AssignmentRouteAgentError):
...
class ProviderCallReconciliationPendingError(AssignmentRouteAgentError):
