"""Technical Review Agent domain errors (ADR-0007 Section 2, ADR-0022).
...
`ai_platform.agents.data_analysis_agent.errors` (renamed base exception)
...
class TechnicalReviewAgentError(Exception):
    """Base class for Technical Review Agent domain errors."""


class CapabilityMismatchError(TechnicalReviewAgentError):
...
class CommandIdentityConflictError(TechnicalReviewAgentError):
...
class CommandIntegrityError(TechnicalReviewAgentError):
...
class MissingOutcomeInvariantError(TechnicalReviewAgentError):
...
class ProviderCallReconciliationPendingError(TechnicalReviewAgentError):
