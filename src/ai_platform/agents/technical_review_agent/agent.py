"""The Technical Review Agent execution lifecycle (ADR-0022, mirroring
`ai_platform.agents.data_analysis_agent.agent`).

Ordering, the durable pre-call claim, and the ADR-0016 unknown-outcome
handling are identical to `DataAnalysisAgent` -- see that module's
docstring for the full rationale, which applies unchanged here. The one
difference: `technical.review`'s findings use a free-text `component`
locator instead of `data.analysis`'s `metric`, since a reviewed technical
design has no metric concept -- the same adaptation `data_analysis_agent`
made from `architecture_review_agent`'s `section`. A successful provider
call is not automatically a successful completion here either: the raw
response text must still parse into a valid findings list (see
...
from ai_platform.agents.technical_review_agent.errors import (
...
from ai_platform.agents.technical_review_agent.ids import IdentifierFactory
...
_MALFORMED_OUTPUT_FAILURE_CODE = "MALFORMED_TECHNICAL_REVIEW_OUTPUT"
_VALID_SEVERITIES = frozenset({"low", "medium", "high"})
_MAX_FINDING_SUMMARY_LENGTH = 2000
_MAX_FINDING_COMPONENT_LENGTH = 200
_MAX_FINDINGS = 100
_REQUIRED_FINDING_KEYS = frozenset({"component", "summary", "severity"})


class TechnicalReviewAgentDisposition(Enum):
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
...
class TechnicalReviewAgentResult:
    disposition: TechnicalReviewAgentDisposition
...
def _build_review_prompt(design: str) -> str:
    return (
        "You are a meticulous technical architect. Review the following "
        "proposed data model/schema, API/contract definition, service-"
        "boundary design, or deployment-topology description and respond "
        "with ONLY a JSON array (no prose, no markdown fences) of finding "
        "objects. Each object must have exactly these keys: "
        '"component" (string, a short label for the module, schema, '
        "endpoint, or service boundary the finding refers to, e.g. "
        '"users table", "POST /api/v1/workflows", "Kafka topic '
        'partitioning"), "summary" (string, one sentence describing the '
        'concern), and "severity" (one of "low", "medium", "high"). '
        "Focus on whether the design is sound and buildable as specified "
        "-- schema/contract consistency, service-boundary clarity, "
        "deployment-topology soundness -- not prose style. Return an "
        "empty array [] if there are no findings."
        f"\n\nDesign:\n{design}"
    )
...
def _parse_findings(output_text: str) -> list[dict[str, object]] | None:
    ...
    as a failed completion (`MALFORMED_TECHNICAL_REVIEW_OUTPUT`), never as
...
class TechnicalReviewAgent:
    ...
    async def handle(
        ...
        if context.task_result_deadline <= now:
            ...
            disposition = TechnicalReviewAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
            ...
            if not claim.created:
                ...
                if completion.output_text is not None:
                    ...
                    if findings is not None:
                        ...
                        disposition = TechnicalReviewAgentDisposition.COMPLETED
                    else:
                        outcome = AgentOutcome(
                            task_attempt_id=context.task_attempt_id,
                            completed_at=now,
                        ...
                        disposition = TechnicalReviewAgentDisposition.FAILED
                else:
        ...
        return TechnicalReviewAgentResult(
    ...
    def _resolve_duplicate(
        ...
        return TechnicalReviewAgentResult(
            disposition=TechnicalReviewAgentDisposition.DUPLICATE_RESOLVED,
