"""Summarize Agent domain errors (ADR-0007 Section 2, ADR-0014 Section 5).
...
`ai_platform.agents.test_agent.errors` (renamed base exception) rather than
imported, per this module's self-contained-deployable convention -- see
`ids.py` for the same rationale.
"""
...
class SummarizeAgentError(Exception):
    """Base class for Summarize Agent domain errors."""


class CapabilityMismatchError(SummarizeAgentError):
...
class CommandIdentityConflictError(SummarizeAgentError):
...
class CommandIntegrityError(SummarizeAgentError):
...
class MissingOutcomeInvariantError(SummarizeAgentError):
