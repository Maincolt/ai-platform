"""UI Review Agent domain errors (ADR-0007 Section 2, ADR-0019).

Stable, transport-independent classifications. Not HTTP errors or Problem
Details -- those belong to the Workflow API (Phase 5). The first five
classes are duplicated from `ai_platform.agents.review_agent.errors`
(renamed base exception) rather than imported, per this module's
self-contained-deployable convention -- see `ids.py` for the same
rationale. `DisallowedReviewTargetError` and `CaptureFailedError` are new,
covering the two failure modes specific to this capability's pre-AI-call
capture phase (ADR-0019 Decision 1/4).
"""

from ai_platform.shared.identifiers import MessageId, TaskAttemptId


class UiReviewAgentError(Exception):
    """Base class for UI Review Agent domain errors."""


class CapabilityMismatchError(UiReviewAgentError):
...
class CommandIdentityConflictError(UiReviewAgentError):
...
class CommandIntegrityError(UiReviewAgentError):
...
class MissingOutcomeInvariantError(UiReviewAgentError):
...
class ProviderCallReconciliationPendingError(UiReviewAgentError):
...
class DisallowedReviewTargetError(UiReviewAgentError):
    """The command's input URL does not exactly match the hardcoded
    review target (ADR-0019 Decision 4).

    Raised before any provider-call claim is taken and before Playwright
    ever navigates -- a disallowed target is a fail-closed input-validation
    rejection, not a capability side effect.
    """

    def __init__(self, requested_target: str) -> None:
        self.requested_target = requested_target
        super().__init__(f"review target {requested_target!r} is not the allowed review target")


class CaptureFailedError(UiReviewAgentError):
    """The Playwright capture step failed (navigation error, timeout, or a
    redirect off the allowed host -- ADR-0019 Decision 4).

    Raised before any provider-call claim is taken: no AI Router call
    happens if the page was never successfully captured.
    """

    def __init__(self, reason: str) -> None:
        self.reason = reason
        super().__init__(f"page capture failed: {reason}")
