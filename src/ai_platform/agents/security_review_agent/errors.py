"""Security Review Agent domain errors (ADR-0007 Section 2, ADR-0025).
...
`ai_platform.agents.technical_review_agent.errors` (renamed base exception)
...
class SecurityReviewAgentError(Exception):
    """Base class for Security Review Agent domain errors."""


class CapabilityMismatchError(SecurityReviewAgentError):
...
class CommandIdentityConflictError(SecurityReviewAgentError):
...
class CommandIntegrityError(SecurityReviewAgentError):
...
class MissingOutcomeInvariantError(SecurityReviewAgentError):
...
class ProviderCallReconciliationPendingError(SecurityReviewAgentError):
