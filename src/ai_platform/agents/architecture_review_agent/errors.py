"""Architecture Review Agent domain errors (ADR-0007 Section 2, ADR-0020).
...
`ai_platform.agents.review_agent.errors` (renamed base exception) rather
than imported, per this module's self-contained-deployable convention --
see `ids.py` for the same rationale.
"""

from ai_platform.shared.identifiers import MessageId, TaskAttemptId


class ArchitectureReviewAgentError(Exception):
    """Base class for Architecture Review Agent domain errors."""


class CapabilityMismatchError(ArchitectureReviewAgentError):
...
class CommandIdentityConflictError(ArchitectureReviewAgentError):
...
class CommandIntegrityError(ArchitectureReviewAgentError):
...
class MissingOutcomeInvariantError(ArchitectureReviewAgentError):
...
class ProviderCallReconciliationPendingError(ArchitectureReviewAgentError):
