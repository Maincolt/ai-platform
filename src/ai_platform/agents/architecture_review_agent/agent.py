"""The Architecture Review Agent execution lifecycle (ADR-0020, mirroring
...
the full rationale, which applies unchanged here. The one difference:
`architecture.review`'s findings use a free-text `section` locator instead
of `code.review`'s `file`/`line` pair, since a reviewed document has no
file/line concept -- the same adaptation `ui_review_agent` made with its
`area` key. A successful provider call is not automatically a successful
completion here either: the raw response text must still parse into a
valid findings list (see `_parse_findings` below), same strict-parse
discipline as `code.review`/`ui.review`.
"""
...
from ai_platform.agents.architecture_review_agent.errors import (
...
from ai_platform.agents.architecture_review_agent.ids import IdentifierFactory
...
_MALFORMED_OUTPUT_FAILURE_CODE = "MALFORMED_ARCHITECTURE_REVIEW_OUTPUT"
_VALID_SEVERITIES = frozenset({"low", "medium", "high"})
_MAX_FINDING_SUMMARY_LENGTH = 2000
_MAX_FINDING_SECTION_LENGTH = 200
_MAX_FINDINGS = 100
_REQUIRED_FINDING_KEYS = frozenset({"section", "summary", "severity"})


class ArchitectureReviewAgentDisposition(Enum):
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
...
class ArchitectureReviewAgentResult:
    disposition: ArchitectureReviewAgentDisposition
...
def _build_review_prompt(proposal: str) -> str:
    return (
        "You are a meticulous solution architect. Review the following "
        "proposed architectural change, design document, or ADR draft and "
        "respond with ONLY a JSON array (no prose, no markdown fences) of "
        "finding objects. Each object must have exactly these keys: "
        '"section" (string, a short label for the part of the document '
        'the finding refers to, e.g. "Decision 2", "Consequences", '
        '"Security"), "summary" (string, one sentence describing the '
        'concern), and "severity" (one of "low", "medium", "high"). '
        "Focus on architectural risk, consistency with existing "
        "decisions, scope creep, and completeness -- not prose style. "
        f"Return an empty array [] if there are no findings.\n\nDocument:\n{proposal}"
    )
...
def _parse_findings(output_text: str) -> list[dict[str, object]] | None:
    ...
    as a failed completion (`MALFORMED_ARCHITECTURE_REVIEW_OUTPUT`), never
    as a partial or best-effort result.
    """
    try:
    ...
    for candidate in candidates:
        ...
        if (
            ...
            or not section_value
            or len(section_value) > _MAX_FINDING_SECTION_LENGTH
        ):
            return None
        if (
...
class ArchitectureReviewAgent:
    ...
    async def handle(
        ...
        if context.task_result_deadline <= now:
            ...
            disposition = ArchitectureReviewAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
            ...
            if not claim.created:
                ...
                if completion.output_text is not None:
                    ...
                    if findings is not None:
                        ...
                        disposition = ArchitectureReviewAgentDisposition.COMPLETED
                    else:
                        outcome = AgentOutcome(
                            task_attempt_id=context.task_attempt_id,
                            completed_at=now,
                        ...
                        disposition = ArchitectureReviewAgentDisposition.FAILED
                else:
        ...
        return ArchitectureReviewAgentResult(
    ...
    def _resolve_duplicate(
        ...
        return ArchitectureReviewAgentResult(
            disposition=ArchitectureReviewAgentDisposition.DUPLICATE_RESOLVED,
