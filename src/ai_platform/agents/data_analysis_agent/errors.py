"""Data Analysis Agent domain errors (ADR-0007 Section 2, ADR-0021).
...
`ai_platform.agents.architecture_review_agent.errors` (renamed base
...
class DataAnalysisAgentError(Exception):
    """Base class for Data Analysis Agent domain errors."""


class CapabilityMismatchError(DataAnalysisAgentError):
...
class CommandIdentityConflictError(DataAnalysisAgentError):
...
class CommandIntegrityError(DataAnalysisAgentError):
...
class MissingOutcomeInvariantError(DataAnalysisAgentError):
...
class ProviderCallReconciliationPendingError(DataAnalysisAgentError):
