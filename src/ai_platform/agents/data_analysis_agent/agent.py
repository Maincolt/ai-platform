"""The Data Analysis Agent execution lifecycle (ADR-0021, mirroring
`ai_platform.agents.architecture_review_agent.agent`).

Ordering, the durable pre-call claim, and the ADR-0016 unknown-outcome
handling are identical to `ArchitectureReviewAgent` -- see that module's
docstring for the full rationale, which applies unchanged here. The one
difference: `data.analysis`'s findings use a free-text `metric` locator
instead of `architecture.review`'s `section`, since a reviewed dataset or
metrics report has no section concept -- the same adaptation
`architecture_review_agent` made from `review_agent`'s `file`/`line` pair.
A successful provider call is not automatically a successful completion
here either: the raw response text must still parse into a valid findings
list (see `_parse_findings` below), same strict-parse discipline as every
...
from ai_platform.agents.data_analysis_agent.errors import (
...
from ai_platform.agents.data_analysis_agent.ids import IdentifierFactory
...
_MALFORMED_OUTPUT_FAILURE_CODE = "MALFORMED_DATA_ANALYSIS_OUTPUT"
_VALID_SEVERITIES = frozenset({"low", "medium", "high"})
_MAX_FINDING_SUMMARY_LENGTH = 2000
_MAX_FINDING_METRIC_LENGTH = 200
_MAX_FINDINGS = 100
_REQUIRED_FINDING_KEYS = frozenset({"metric", "summary", "severity"})


class DataAnalysisAgentDisposition(Enum):
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
...
class DataAnalysisAgentResult:
    disposition: DataAnalysisAgentDisposition
...
def _build_review_prompt(dataset: str) -> str:
    return (
        "You are a meticulous data analyst. Review the following dataset "
        "excerpt, metrics summary, or usage/cost report and respond with "
        "ONLY a JSON array (no prose, no markdown fences) of finding "
        "objects. Each object must have exactly these keys: "
        '"metric" (string, a short label for the metric, data point, or '
        'observation the finding refers to, e.g. "p95 latency", "monthly '
        'active users", "AI provider cost"), "summary" (string, one '
        'sentence describing the observation or concern), and "severity" '
        '(one of "low", "medium", "high"). Focus on trends, anomalies, '
        "data quality issues, and correlations worth investigating -- not "
        "prose style. Return an empty array [] if there are no findings."
        f"\n\nData:\n{dataset}"
    )
...
def _parse_findings(output_text: str) -> list[dict[str, object]] | None:
    ...
    as a failed completion (`MALFORMED_DATA_ANALYSIS_OUTPUT`), never as a
    ...
    for candidate in candidates:
        ...
        if (
            ...
            or not metric_value
            or len(metric_value) > _MAX_FINDING_METRIC_LENGTH
        ):
            return None
        if (
...
class DataAnalysisAgent:
    ...
    async def handle(
        ...
        if context.task_result_deadline <= now:
            ...
            disposition = DataAnalysisAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION
            ...
            if not claim.created:
                ...
                if completion.output_text is not None:
                    ...
                    if findings is not None:
                        ...
                        disposition = DataAnalysisAgentDisposition.COMPLETED
                    else:
                        outcome = AgentOutcome(
                            task_attempt_id=context.task_attempt_id,
                            completed_at=now,
                        ...
                        disposition = DataAnalysisAgentDisposition.FAILED
                else:
        ...
        return DataAnalysisAgentResult(
    ...
    def _resolve_duplicate(
        ...
        return DataAnalysisAgentResult(
            disposition=DataAnalysisAgentDisposition.DUPLICATE_RESOLVED,
