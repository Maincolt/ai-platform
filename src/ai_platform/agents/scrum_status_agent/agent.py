"""The Scrum Status Agent execution lifecycle (ADR-0027, mirroring
`ai_platform.agents.ui_review_agent.agent`).

Ordering, the durable pre-call claim, and the ADR-0016 unknown-outcome
handling are identical to `UiReviewAgent` -- see that module's docstring
for the full rationale, which applies unchanged here. One addition,
happening *before* any provider-call claim is taken (a failed fetch means
no AI Router call happens at all, so nothing needs reconciling if it
occurs): a deterministic, read-only project-board fetch (`board.py`) must
succeed before the single AI Router call is made -- a fetch failure fails
closed with `PROJECT_BOARD_FETCH_FAILED`. Unlike `UiReviewAgent`, there is
no disallowed-target check: the board to fetch is server-side
configuration (ADR-0027 Decision 4), not caller-supplied input, so there
is nothing about the input to validate before fetching.

From the claim onward, this mirrors `UiReviewAgent` exactly: `ui.review`'s
`_parse_ui_findings` becomes `_parse_findings` (same strict-parse
discipline: any shape/content mismatch returns `None`, treated as
`MALFORMED_SCRUM_STATUS_OUTPUT`, never a partial or best-effort result),
and the findings shape drops `area` in favor of a free-text `location`
locator, matching every non-`ui.review` capability's naming.
"""
...
from ai_platform.agents.scrum_status_agent.board import ProjectBoardPort, ProjectBoardSnapshot
from ai_platform.agents.scrum_status_agent.capability import CAPABILITY_NAME, CAPABILITY_VERSION
...
from ai_platform.agents.scrum_status_agent.ids import IdentifierFactory
...
_FETCH_FAILED_FAILURE_CODE = "PROJECT_BOARD_FETCH_FAILED"
_MALFORMED_OUTPUT_FAILURE_CODE = "MALFORMED_SCRUM_STATUS_OUTPUT"
...
class ScrumStatusAgentDisposition(Enum):
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
...
class ScrumStatusAgentResult:
    disposition: ScrumStatusAgentDisposition
...
def _build_status_prompt(snapshot: ProjectBoardSnapshot, focus: str) -> str:
    item_lines = (
        "\n".join(
            f"- {item.title} [{item.status or 'no status'}]"
            + (f" ({item.url})" if item.url else "")
            for item in snapshot.items
        )
        or "(no items)"
    )
    focus_line = focus.strip() or "(no specific focus requested -- give a general status summary)"
    return (
        "You are a meticulous scrum status reporter. Review the following "
        "live project board snapshot and respond with ONLY a JSON array "
        "(no prose, no markdown fences) of finding objects. Each object "
        'must have exactly these keys: "location" (string, a short label '
        "for the issue/card/metric the observation refers to, e.g. "
        '"issue #42 (In Progress)", "Sprint velocity"), "summary" '
        "(string, one sentence describing the observation), and "
        '"severity" (one of "low", "medium", "high" -- how much '
        "attention the observation deserves). Focus on blockers, stale "
        "items, missing assignees, and anything relevant to the caller's "
        "focus below. Return an empty array [] if there is nothing "
        "notable.\n\n"
        f"Caller's focus: {focus_line}\n\n"
...
def _strip_markdown_json_fence(text: str) -> str:
    ...
    Anthropic model for `code.review`/`ui.review` -- see ADR-0019's
    Implementation Status and PR #40). This is a presentation-format
...
def _parse_findings(output_text: str) -> list[dict[str, object]] | None:
    ...
    as a failed completion (`MALFORMED_SCRUM_STATUS_OUTPUT`), never as a
...
class ScrumStatusAgent:
    def __init__(
        ...
        *,
        ...
        project_board: ProjectBoardPort,
        ...
        self._project_board = project_board
        self._max_output_tokens = max_output_tokens

    async def handle(self, context: ExecuteTaskContext, *, now: datetime) -> ScrumStatusAgentResult:
        if (
        ...
        if context.task_result_deadline <= now:
            ...
            return await self._commit(
                context,
                outcome,
                disposition=ScrumStatusAgentDisposition.DEADLINE_EXPIRED_BEFORE_EXECUTION,
                usage=None,
                now=now,
            )

        try:
            snapshot = await self._project_board.fetch()
        except ProjectBoardFetchFailedError as error:
            outcome = AgentOutcome(
                task_attempt_id=context.task_attempt_id,
                completed_at=now,
                failure_code=_FETCH_FAILED_FAILURE_CODE,
                summary=f"Project board fetch failed: {error.reason}",
            )
            return await self._commit(
                context,
                outcome,
                disposition=ScrumStatusAgentDisposition.FAILED,
                usage=None,
                now=now,
            )
        ...
        completion = await self._ai_router.complete(
            AICompletionRequest(
                prompt=_build_status_prompt(snapshot, context.input_text),
        ...
        if completion.output_text is not None:
            ...
            if findings is not None:
                ...
                disposition = ScrumStatusAgentDisposition.COMPLETED
            else:
                outcome = AgentOutcome(
                    task_attempt_id=context.task_attempt_id,
                    completed_at=now,
    ...
    def _resolve_duplicate(
        ...
        return ScrumStatusAgentResult(
            disposition=ScrumStatusAgentDisposition.DUPLICATE_RESOLVED,
