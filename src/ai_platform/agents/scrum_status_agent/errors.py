"""Scrum Status Agent domain errors (ADR-0007 Section 2, ADR-0027).
...
classes are duplicated from `ai_platform.agents.ui_review_agent.errors`
...
rationale. `ProjectBoardFetchFailedError` is new, covering the one
failure mode specific to this capability's pre-AI-call fetch phase
(ADR-0027 Decision 3).
"""
...
class ScrumStatusAgentError(Exception):
    """Base class for Scrum Status Agent domain errors."""


class CapabilityMismatchError(ScrumStatusAgentError):
...
class CommandIdentityConflictError(ScrumStatusAgentError):
...
class CommandIntegrityError(ScrumStatusAgentError):
...
class MissingOutcomeInvariantError(ScrumStatusAgentError):
...
class ProviderCallReconciliationPendingError(ScrumStatusAgentError):
...
class ProjectBoardFetchFailedError(ScrumStatusAgentError):
    """The project-board fetch step failed (HTTP error, timeout, GraphQL
    error response, or malformed shape -- ADR-0027 Decision 3).

    Raised before any provider-call claim is taken: no AI Router call
    happens if the board was never successfully fetched.
    """
